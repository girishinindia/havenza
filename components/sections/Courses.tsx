...
import { COURSES, COURSE_FORMATS, type CourseFormat } from '@/lib/data';
...
export default function Courses() {
  ...
  const list = COURSES.filter((c) => format === 'all' || c.formats.includes(format));
    ...
    <section id="learn" className="relative py-20 lg:py-28 scroll-mt-20">
      <div className="max-w-site mx-auto px-5 lg:px-10">
        ...
        <div className="reveal mt-10 flex gap-2 overflow-x-auto snap-row pb-2 -mx-5 px-5 lg:mx-0 lg:px-0" role="tablist" aria-label="Filter courses">
          {[{ key: 'all' as const, label: 'All courses' }, ...COURSE_FORMATS].map((f) => (
          ...
          {list.map((c, i) => (
            <article key={`${format}-${c.id}`} className="lux-card reveal group p-7 lg:p-8 flex flex-col" style={d((i % 3) * 70)}>
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="text-[10.5px] tracking-[.2em] uppercase text-rose-700 bg-blush-100 rounded-full px-3 py-1.5">{c.level}</span>
              </div>
              ...
              <div className="mt-auto pt-7">
                ...
                <div className="pt-5 flex items-center justify-between text-[11.5px] tracking-[.2em] uppercase">
                  ...
                  <button type="button" onClick={() => prefillEnquiry(c.name, c.id === 'teams' ? 'business' : 'personal')} className="flex items-center gap-1.5 text-rose-700 group-hover:gap-3 transition-all uppercase tracking-[.2em]">
